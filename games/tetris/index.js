...
var ASM_CONSTS = {
  87800: ($0) => { var score = $0; console.log("C++: 尝试提交分数 " + score); if (typeof UpdateWebScore === 'function') { UpdateWebScore(score); } else if (window.parent && typeof window.parent.UpdateWebScore === 'function') { window.parent.UpdateWebScore(score); } else { console.warn("未找到 UpdateWebScore 函数"); } },  
 88115: () => { window.addEventListener("keydown", function(e) { if([32, 37, 38, 39, 40].indexOf(e.keyCode) > -1) { e.preventDefault(); } }, false); console.log("TinyPulse: 已成功锁定浏览器滚动行为"); }
};
